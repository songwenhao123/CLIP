...
def get_dataloader(dataset, is_train, image_encoder=None):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(is_train, image_encoder, dataset, device = "cuda" if torch.cuda.is_available() else "cpu")
        dataloader = DataLoader(feature_dataset, batch_size=64, shuffle=is_train)
    else:
